...
{
    internal static string? ProductKeyFromRegistry
    {
        get
        {
    ...
    {
        if (digitalProductIdVersion is DigitalProductIdVersion.Windows8AndUp)
        {
    ...
    {
        char[] digits = KeyDecoder.digits.ToCharArray(),
               decodedChars = new char[decodeLength];
    ...
    static string DecodeProductKeyWin8AndUp(byte[] digitalProductId)
    {
        ...
        string keypart1 = key.Substring(1, last),
               keypart2 = key.Substring(1 + last, key.Length - (last + 1));
